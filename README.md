...
Switch between two programs (a per-user toggle in the Train header): a **Gym** 3-day **Push / Pull / Legs** split, or a **Home (Kettlebell)** 3-day full-body split (KB Push / Pull / Conditioning) for a minimal one- or two-bell setup. Each program's sessions are stored separately, and history/Overview resolves exercises from either. The PPL split eases in (lighter leg day, conservative starting volume). Each training day includes a **required cardio block** — easy steady-state (Z2) on Push/Pull, high-intensity intervals (Z4) on Legs; a session isn't complete until cardio has a duration logged (you can still End workout early). Log sets with an auto rest timer, see last session and estimated 1RM with a live PR flag, inline per-exercise trend sparklines, exercise substitutes and editable set/rep schemes, and a cross-exercise **Overview** with bodyweight, cardio, and Recent Sessions analytics.
